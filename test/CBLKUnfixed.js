...
describe('CBLKUnfixed', () => {
  ...
  describe('Rebalance', async () => {
    it('Correct state post inital CBT deposit', async () => {
      expect(await CBT1.balanceOf(CBLKUnfixed.address)).to.equal(
        ethers.utils.parseEther('1000')
      );
      expect(await CBT2.balanceOf(CBLKUnfixed.address)).to.equal(
        ethers.utils.parseEther('1000')
      );
      expect(await CBT3.balanceOf(CBLKUnfixed.address)).to.equal(
        ethers.utils.parseEther('1000')
      );
    ...
    it('Introduction of new token', async () => {
      ...
      expect(await CBT4.balanceOf(CBLKUnfixed.address)).to.equal(
        ethers.utils.parseEther('1000')
      );
  ...
  describe('Withdraw', () => {
    ...
    it('Partial withdrawal', async () => {
      ...
      expect(await CBLKUnfixed.totalSupply()).to.equal(
        ethers.utils.parseEther('1750')
      );
    });
    ...
    it('Rejects withdrawal with insufficient CBLK', async () => {
      await expect(
        CBLKUnfixed.connect(notDeployer).withdraw(
          ethers.utils.parseEther('1000')
        )
      ).to.be.revertedWith('ERC20: burn amount exceeds balance');
      await expect(
        CBLKUnfixed.withdraw(ethers.utils.parseEther('4000'))
      ).to.be.revertedWith('ERC20: burn amount exceeds balance');
    });
